...
describe("createProduct", () => {
  ...
  it("auto-generates a SKU when none provided", async () => {
    mockClient._setResult({
      data: { id: "prod-2", name: "Test", sku: "TEST-123456" },
      error: null,
    });
  ...
  it("throws on Supabase error", async () => {
    ...
    await expect(createProduct({ organization_id: "org-1", name: "Fail" })).rejects.toThrow(
      "Insert failed"
    );
  });
...
describe("getProductsStats", () => {
  ...
  it("counts product at exactly stock_min as lowStock", async () => {
    const products = [{ stock_current: 10, stock_min: 10, price: 5 }];
    mockClient._setResult({ data: products, error: null });
...
describe("getProducts", () => {
  ...
  it("filters by search term", async () => {
    ...
    expect(mockClient.or).toHaveBeenCalledWith(expect.stringContaining("peinture"));
  });
...
describe("getProducts stockStatus filtering", () => {
  const mockProducts = [
    { id: "p1", name: "Low Stock", stock_current: 3, stock_min: 10 },
    { id: "p2", name: "Normal", stock_current: 50, stock_min: 10 },
    { id: "p3", name: "High Stock", stock_current: 100, stock_min: 10 },
    { id: "p4", name: "Exact Min", stock_current: 10, stock_min: 10 },
    { id: "p5", name: "Above Min", stock_current: 15, stock_min: 10 },
  ];
...
describe("deleteProductImage", () => {
  it("extracts path and removes file from storage", async () => {
    ...
    await deleteProductImage(
      "https://example.com/storage/v1/object/public/product-images/products/test.jpg"
    );
