-- Quantite corrigee, portee par le mouvement d'origine.
--
-- Sans elle, une correction n'existe que sous forme d'une ligne separee, et
-- tout agregat qui somme les mouvements se trompe deux fois :
--   - corriger une SORTIE cree une ligne 'entry' comptee comme un achat,
--     alors que rien n'a ete achete ;
--   - corriger une ENTREE laisse l'entree d'origine intacte, donc les achats
--     affichent toujours la quantite fautive.
--
-- En inscrivant la quantite corrigee sur le mouvement lui-meme, sa quantite
-- reelle devient (quantity - reversed_quantity) et les agregats n'ont plus
-- qu'a ecarter les lignes de correction.
ALTER TABLE stock_movements
  ADD COLUMN IF NOT EXISTS reversed_quantity integer NOT NULL DEFAULT 0;

COMMENT ON COLUMN stock_movements.reversed_quantity IS
  'Quantite deja corrigee sur ce mouvement. Quantite reelle = quantity - reversed_quantity.';

-- Reprise des donnees existantes : aucune correction n'a encore ete faite en
-- production, mais la requete reste juste si le cas se presentait.
UPDATE stock_movements m
SET reversed_quantity = COALESCE(r.total, 0)
FROM (
  SELECT reverses_movement_id AS id, SUM(quantity) AS total
  FROM stock_movements WHERE reverses_movement_id IS NOT NULL
  GROUP BY reverses_movement_id
) r
WHERE m.id = r.id AND m.reversed_quantity <> COALESCE(r.total, 0);

-- La fonction de correction tient desormais ce compteur a jour.
DROP FUNCTION IF EXISTS reverse_stock_movement(uuid, integer);
...
BEGIN
  ...
  v_qty := COALESCE(p_quantity, v_mv.quantity - v_mv.reversed_quantity);

  IF v_qty <= 0 THEN
    RAISE EXCEPTION 'La quantite a corriger doit etre positive';
  END IF;

  IF v_mv.reversed_quantity + v_qty > v_mv.quantity THEN
    RAISE EXCEPTION 'Correction impossible : % deja corrigee(s) sur %, il reste % unite(s)',
      v_mv.reversed_quantity, v_mv.quantity, v_mv.quantity - v_mv.reversed_quantity;
  END IF;
  ...
  -- Le mouvement d'origine porte desormais sa part corrigee
  UPDATE stock_movements
    SET reversed_quantity = reversed_quantity + v_qty
  ...
  RETURN jsonb_build_object(
    ...
    'quantity', v_qty,
    'remaining', v_mv.quantity - v_mv.reversed_quantity - v_qty
  );
END;
$$;

COMMENT ON FUNCTION reverse_stock_movement(uuid, integer) IS
  'Corrige tout ou partie d''un mouvement par un mouvement inverse, et inscrit la part corrigee sur le mouvement d''origine.';
