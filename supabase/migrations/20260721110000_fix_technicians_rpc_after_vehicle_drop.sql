-- Correctif : get_technicians_with_stats referencait les colonnes vehicle_*
-- de technicians, supprimees par 20260721100000. La fonction levait
-- « column t.vehicle_plate does not exist » et la page Techniciens ne
-- s'affichait plus.
--
-- Meme signature que l'existante (uuid, integer) : CREATE OR REPLACE remplace
-- donc bien la fonction au lieu de creer une seconde surcharge.
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      ...
      'phone', t.phone,
      'city', t.city,
      -- vehicle_plate / vehicle_brand / vehicle_model retires : le vehicule
      -- se lit desormais dans la table vehicles (vehicles.technician_id).
