...
export function useCreateStockEntry() {
  ...
  return useMutation({
    ...
    onMutate: async (params) => {
      ...
      const previous = qc.getQueryData(queryKeys.products.detail(params.productId));
      qc.setQueryData(queryKeys.products.detail(params.productId), (old: any) =>
        old ? { ...old, stock_current: old.stock_current + params.quantity } : old
      );
      return { previous, productId: params.productId };
    },
    onError: (_err, _params, context) => {
      if (context?.previous) {
        qc.setQueryData(queryKeys.products.detail(context.productId), context.previous);
      }
    },
